...
class Register {
    ...
    public static function existNumberPhone($conn, $numberPhone) {
        $stmt = $conn->prepare("SELECT 1 FROM users WHERE numberPhone = ? LIMIT 1");
        $stmt->bindParam(1, $numberPhone);
        $stmt->execute();
        return $stmt->rowCount() > 0;
    }

    public static function addUser($username, $password, $numberPhone, $province, $district, $detailAddress) {
        ...
        try {
            ...
            $stmt = $conn->prepare("
                INSERT INTO users (userName,password,numberPhone,fullName,status,role)
                VALUES (?,?,?, '', 'active', 'user')
            ");

            $stmt->bindParam(1,$username);
            $stmt->bindParam(2,$password);
            $stmt->bindParam(3,$numberPhone);
            $stmt->execute();
            ...
            $stmt2 = $conn->prepare("
                INSERT INTO address (userId,detailAddress,province,district)
                VALUES (?,?,?,?)
            ");
